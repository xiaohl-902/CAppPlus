...
public class CallAPI {
    ...
    public static org.xutils.common.Callback.Cancelable singleLoginCheck(Context context,
        ...
        params.addQueryStringParameter("p", SharedPreferencesTool.Get(context, "password","",SharedPreferencesTool.SP_FILE_LOGIN));
        ...
        return x.http().get(requestParams, new org.xutils.common.Callback.CommonCallback<String>() {
            ...
            private void dealData(String result) {
                ...
                try {
                        ...
                        if (!obj.isNull("data")) {
                            ...
                            if (array != null) {
                                for (int i = 0; i < array.length(); i++) {
                                    ...
                                    cpigeonOrderInfo.ddtype  = obj.has("ddtype") ? obj.getString("ddtype") : "";
                                    cpigeonOrderInfo.ddly = obj.has("ddly") ? obj.getString("ddly") : "";
                                    cpigeonOrderInfo.fpid = obj.has("fpid") ? obj.getString("fpid") : "";
                    ...
                    /*ApiResponse<List<CpigeonOrderInfo>> apiResponse = GsonUtil.fromJson(result, new TypeToken<ApiResponse<List<CpigeonOrderInfo>>>(){}.getType());
                    if(apiResponse.status){
                        if(!apiResponse.data.isEmpty()){
                            callback.onSuccess(apiResponse.data);
                        }else {
                            callback.onSuccess(Lists.newArrayList());
                        }
                    }else {
...
//                                Logger.i("赛事共计：" + array.length());`
                                ...
                                for (int i = 0; i < array.length(); i++) {
                                    ...
                                    matchInfo.setId(jo.has("rid") ? jo.getInt("rid") : 0);
                                    list.add(matchInfo);
                                }
     ...
     * @param callback
     */
    public static org.xutils.common.Callback.Cancelable getReportData(   int px,
        ...
        requestParams.addParameter("px", px);
